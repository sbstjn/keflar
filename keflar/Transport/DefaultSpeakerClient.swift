...
struct DefaultSpeakerClient: SpeakerClientProtocol, Sendable {
    ...
    private func dataForRequest(_ request: URLRequest) async throws -> (Data, URLResponse) {
        do {
            ...
            Logger.network.error("Request failed: \(error.localizedDescription, privacy: .private)")
    ...
    func getData(path: String) async throws -> [String: Any] {
        ...
        do {
            let result = try await retryPolicy.execute {
                ...
                let start = Date()
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) path=\(path, privacy: .private) GET getData")
                ...
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) path=\(path, privacy: .private) GET getData durationMs=\(elapsedMs)")
                let json: Any
                do {
    ...
    func setDataWithBody<E: Encodable>(path: String, role: String, value: E) async throws {
        ...
        do {
            try await retryPolicy.execute {
                let ctx = RequestLogContext(path: path)
                let start = Date()
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) path=\(path, privacy: .private) POST setData role=\(role)")
                ...
                let elapsedMs = Int(Date().timeIntervalSince(start) * 1000)
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) path=\(path, privacy: .private) POST setData durationMs=\(elapsedMs)")
    ...
    func getRows(path: String, from: Int, to: Int) async throws -> [String: Any] {
        ...
        do {
            let result = try await retryPolicy.execute {
                let ctx = RequestLogContext(path: path)
                let start = Date()
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) path=\(path, privacy: .private) GET getRows from=\(from) to=\(to)")
                ...
                let (data, response) = try await dataForRequest(request)
                let elapsedMs = Int(Date().timeIntervalSince(start) * 1000)
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) path=\(path, privacy: .private) GET getRows durationMs=\(elapsedMs)")
                if let http = response as? HTTPURLResponse, !(200..<300).contains(http.statusCode) {
    ...
    func browse(path: String, from: Int, to: Int) async throws -> BrowseResult {
        var response = try await getRows(path: path, from: from, to: to)
        let redirectTarget = response["rowsRedirect"] as? String
        if let redirect = redirectTarget, !redirect.isEmpty {
            response = try await getRows(path: redirect, from: from, to: to)
            let result = parseBrowseResult(from: response)
            return BrowseResult(rows: result.rows, totalCount: result.totalCount, redirectedFrom: path)
        }
        return parseBrowseResult(from: response)
    }
    ...
    func modifyQueue() async throws -> String {
        ...
        do {
            let result = try await retryPolicy.execute {
                let ctx = RequestLogContext(path: nil)
                let start = Date()
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) POST modifyQueue")
                ...
                let (data, _) = try await dataForRequest(request)
                let elapsedMs = Int(Date().timeIntervalSince(start) * 1000)
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) POST modifyQueue durationMs=\(elapsedMs)")
    ...
    func pollQueue(queueId: String, timeout: TimeInterval) async throws -> [[String: Any]] {
        ...
        do {
            let result = try await retryPolicy.execute {
                let ctx = RequestLogContext(path: nil)
                let start = Date()
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) GET pollQueue queueId=\(queueId, privacy: .private) timeout=\(Int(timeout))")
                ...
                let (data, _) = try await dataForRequest(request)
                let elapsedMs = Int(Date().timeIntervalSince(start) * 1000)
                Logger.network.debug("correlationId=\(ctx.correlationId, privacy: .public) GET pollQueue durationMs=\(elapsedMs)")
                guard let json = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] else {
                    throw SpeakerConnectError.invalidResponseStructure
                }
                return json
