...
/// Deezer implementation of StreamingServiceOperations. Uses Airable proxy paths per SPECS §2.9.
actor DeezerService: StreamingServiceOperations {
    let service = AudioService.deezer
    ...
    private func resolveBase() async throws -> String {
        ...
        do {
            ...
            Logger.streaming.debug("Deezer resolveBase success")
            return base
        } catch {
            Logger.streaming.error("Deezer resolveBase failed: \(error.localizedDescription, privacy: .private)")
            throw error
        }
    }
    ...
    func addFavorite(trackId: String, returnPath: String?) async throws {
        ...
        let r = returnPath ?? "%2Fdeezer%2Flibrary"
    ...
    func isFavorite(trackId: String) async throws -> Bool {
        let base = try await resolveBase()
        let path = "airable:\(base)/deezer/library"
