...
func fetchInitialState(client: any SpeakerClientProtocol, stateStore: StateStore) async {
    ...
    await stateStore.updateStateBatch(pathDicts: batch)
}

/// Fetch initial state using StateSynchronizer (new flow).
func fetchInitialStateWithSynchronizer(client: any SpeakerClientProtocol, stateSynchronizer: StateSynchronizer) async {
    var results: [(String, [String: Any])] = []
    await withTaskGroup(of: (String, SendableDict?).self) { group in
        for apiPath in initialGetDataPaths {
            let pathString = apiPath.path
    ...
    await stateSynchronizer.applyBatch(pathDicts: results)
}
