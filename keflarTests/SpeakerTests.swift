...
private func connectToSpeaker(host: String = defaultHost, awaitInitialState: Bool = true) async throws -> Speaker {
...
struct SpeakerTests {
    ...
    @Test(.disabled("Requires live speaker on network")) func playThenFlipModes() async throws {
        ...
        let speaker: Speaker
        do {
            speaker = try await connectToSpeaker()
        } catch {
        ...
        try await speaker.playPlaylist(service: .tidal, playlistId: tidalPlaylistId)
