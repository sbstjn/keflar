...
final class MockSpeakerClient: SpeakerClientProtocol, @unchecked Sendable {
    ...
    private var browseStub: BrowseResult?
    ...
    private(set) var browseCalls: [(path: String, from: Int, to: Int)] = []
    ...
    func stubBrowse(result: BrowseResult) {
        browseStub = result
    }
    ...
    func browse(path: String, from: Int, to: Int) async throws -> BrowseResult {
        browseCalls.append((path, from, to))
        return browseStub ?? BrowseResult(rows: [], totalCount: nil, redirectedFrom: nil)
    }
